...
// =============================================================================
let runtimeToken: string | null = process.env.API_TOKEN ?? null
...
// =============================================================================
async function loginService(): Promise<string> {
  const c = client()
  ...
  if (!token || typeof token !== 'string') {
    throw new Error(
      `Login OK mais token introuvable dans la réponse (clés testées: accessToken/token/jwt). Réponse: ${JSON.stringify(
        res.data,
      )}`,
    )
  }
...
async function registerServiceIfNeeded(): Promise<void> {
  const c = client()
  try {
...
async function handleLine(line: string) {
  ...
  if (trimmed === 'help') {
    ...
    console.log(
      chalk.gray(
        '  tickets create --title "..." --priority HIGH --workflowId <uuid> [--description "..."] [--tags "a,b"]',
      ),
    )
